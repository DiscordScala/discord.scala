...
case class Channel(
  ...
  def postMessage(m: Message)(implicit client: Client): Future[Either[DiscordException, Message]] = Future {
    id match {
      case Some(i) =>
        RequestUtil.awaitRestRequestFuture(client.apiURL + s"channels/$i/messages", Map("Authorization" -> client.token), RequestMethod.Post, Extraction.decompose(m), Duration.Inf) match {
          case Left(e) => Left(e)
  ...
  def deleteMessage(m: Message)(implicit client: Client): Future[Either[DiscordException, Unit]] = Future {
    m.id match {
      case Some(messageId) =>
        RequestUtil.awaitRestRequestFuture(client.apiURL + s"channels/$id/messages/$messageId", Map("Authorization" -> client.token), RequestMethod.Delete) match {
          case Left(e) => Left(e)
